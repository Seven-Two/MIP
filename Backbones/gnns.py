...
class GIN(nn.Module):
    ...
    def forward(self, g, features):
        e_list = []
        h = self.gat_layers[0](g, features)
        x = F.relu(h)
        logits = self.gat_layers[1](g, x)
        self.second_last_h = logits if len(self.gat_layers) == 1 else h
        # e_list = e_list + e
        return logits

    def forward_p(self, g, features, prompt_c):
        e_list = []
        h, e = self.gat_layers[0](g, features)

        x = F.relu(h)
        logits, e = self.gat_layers[1](g, x)
        logites = prompt_c.add(logits)
        self.second_last_h = logits if len(self.gat_layers) == 1 else h
        e_list = e_list + e
        return logits
...
class GCN(nn.Module):
    def __init__(self, args, assign=False):
        super(GCN, self).__init__()
        if assign:
            dims = [args.d_data] + [100]
        else:
            if args.method in ['imp', 'tpp', 'tcpp', 'fscil', 'mip']:
                dims = [args.d_data] + args.GCN_args['h_dims'] + [args.GCN_args['h_dims'][-1]]
            else:
                dims = [args.d_data] + args.GCN_args['h_dims'] + args.GCN_args['h_dims'] + [args.n_cls]
        self.dropout = args.GCN_args['dropout']
        self.gat_layers = nn.ModuleList()
        for l in range(len(dims)-1):
            self.gat_layers.append(GCNLayer(dims[l], dims[l+1])) if (args.method == 'puma') else self.gat_layers.append(DGLGCN(dims[l], dims[l+1]))
        args.hidden = args.GCN_args['h_dims'][-1]

    def forward(self, g, features, twp=False, edge_weight=None):
        elist = []
        h = features
        ...
        if twp:
            logits, e = self.gat_layers[-1](g, h, twp)
        else:
    ...
    def forward_p(self, g, features, prompt_c):
        ...
        for i, layer in enumerate(self.gat_layers[:-1]):
            ...
            if i == 0:
                h_prompt = h
                h, _, p = prompt_c.add(h)
            h = F.relu(h)
            h = F.dropout(h, p=self.dropout, training=self.training)
        logits = self.gat_layers[-1](g, h)
        self.second_last_h = logits if len(self.gat_layers) == 1 else h
        return logits, h_prompt, p
...
class SGC(nn.Module):
    ...
    def forward(self, graph, x, twp=False):
        # ipdb.set_trace()
        graph = graph.local_var().to('cuda:{}'.format(self.gpu))
        x = self.neighbor_agg(graph, x)
        if twp:
            logits, elist, _, _ = self.feat_trans(graph, x, twp)
        else:
            logits, _, _ = self.feat_trans(graph, x)

        self.second_last_h = logits if len(self.feat_trans_layers) == 1 else x
        if twp:
            return logits, elist
        return logits

    def forward_p(self, graph, x, prompt_c):
        graph = graph.local_var().to('cuda:{}'.format(self.gpu))
        x = self.neighbor_agg(graph, x)
        logits, h_prompt, p = self.feat_trans(graph, x, prompt_c=prompt_c)
        self.second_last_h = logits if len(self.feat_trans_layers) == 1 else x
        return logits, h_prompt, p


    def forward_batch(self, blocks, x, twp=False):
        #graph = graph.local_var().to('cuda:{}'.format(self.gpu))
        e_list = []

        x = self.neighbor_agg.forward_batch(blocks, x)
        if twp:
            logits, elist, _, _ = self.feat_trans(blocks[0], x, twp)
        else:
            logits, _, _ = self.feat_trans(blocks[0], x)

        if twp:
            return logits, elist
        return logits

    def feat_trans(self, g, x, twp=False, prompt_c=None):
        h_prompt, p = None, None
        elist = []
        for i, layer in enumerate(self.feat_trans_layers[:-1]):
            x = layer(x)
            if self.bn:
                x = self.bns[i](x)
            if i == 0 and prompt_c is not None:
                h_prompt = x
                x, _, p = prompt_c.add(x)

            if twp:
                g.srcdata['h'] = x
                g.apply_edges(lambda edges: {'e': th.sum((th.mul(edges.src['h'], th.tanh(edges.dst['h']))), 1)})
                e =g.edata.pop('e')
