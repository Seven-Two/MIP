...
class GCNLayer(nn.Module):
    ...
    def forward(self, graph, feat, twp=False, edge_weight=None):
        ...
        if edge_weight is not None:
            ...
            gcn_msg = fn.u_mul_e("h", "_edge_weight", "m")
        elist = []
        graph = graph.local_var().to('cuda:{}'.format(feat.get_device()))
        h = self.linear(feat)
        graph.ndata['h'] = h
...
class DGLGCN(nn.Module):
    ...
    def forward(self, graph, feat, twp=False, weight=None, edge_weight=None):
        elist = []
