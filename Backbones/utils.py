...
import dgl
from dgl.data import CoraGraphDataset, CoraFullDataset, register_data_args, RedditDataset, AmazonCoBuyComputerDataset, AmazonCoBuyPhotoDataset, CoauthorCSDataset, RomanEmpireDataset, FlickrDataset, CiteseerGraphDataset
...
def evaluate_taskIL(model, g, features, labels, mask, label_offset1, label_offset2, cls_balance=True, ids_per_cls=None, save_logits_name=None):
    model.eval()
    with torch.no_grad():
        output = model(g, features)
        logits = output[:, label_offset1:label_offset2]
...
class NodeLevelDataset(incremental_graph_trans_):
    def __init__(self,name='ogbn-arxiv',IL='class',default_split=False,ratio_valid_test=None,args=None):
        ...
        if name[0:4] == 'ogbn':
            ...
            data = DglNodePropPredDataset('ogbn-arxiv', f'{args.ori_data_path}/ogb_downloaded')
            graph, label = data[0]
        elif name == 'Products-CL':
            data = DglNodePropPredDataset('ogbn-products', root=f'{args.ori_data_path}/ogb_downloaded')
            graph, label = data[0]
        elif name.startswith("AmazonC"):
            data = AmazonCoBuyComputerDataset()
            graph, label = data[0], data[0].dstdata['label'].view(-1, 1)
        elif name.startswith("AmazonP"):
            data = AmazonCoBuyPhotoDataset()
            graph, label = data[0], data[0].dstdata['label'].view(-1, 1)
        elif name.startswith('Coauthor'):
            data = CoauthorCSDataset()
            graph, label = data[0], data[0].dstdata['label'].view(-1, 1)
        elif name.startswith('Roman'):
            data = RomanEmpireDataset(raw_dir=f'{args.ori_data_path}/romain')
            graph, label = data[0], data[0].dstdata['label'].view(-1, 1)
            print(graph)
        elif name.startswith('Flickr'):
            data = FlickrDataset()
