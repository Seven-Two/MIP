...
class GPFclass(nn.Module):
    def __init__(self, in_channels: int, p_num: int):
        super(GPFclass, self).__init__()
        self.p_list = nn.Parameter(torch.Tensor(p_num, in_channels))
        self.a = nn.Linear(in_channels, p_num)
        self.reset_parameters()
...
class NET(torch.nn.Module):
    ...
    def observe_il(self, g, features, labels, t, train_ids, ids_per_cls, offset1, dataset):
        ...
        if t > 0:
            ...
            prompt_t.train()
            prompt_t.zero_grad()
            features, x, p = prompt_t.add(features)
    ...
    def getpred(self, g, features, taskid, test_ids, labels):
        ...
        if taskid == 0:
            ...
            #     # 1. 计算有效秩 (Effective Rank / Stable Rank)
            #     # 公式: (sum(sigma_i))^2 / sum(sigma_i^2)
            #     # 物理含义：这也叫“稳定秩”。如果 Collapse，该值会接近 1；如果多样性好，该值会远大于 1。
            #     effective_rank = (torch.sum(S) ** 2) / torch.sum(S ** 2)
                
            #     # 2. 计算解释方差比 (Explained Variance Ratio)
            #     # 物理含义：前几个主成分占据了多少能量。如果 top-1 占比 > 90%，说明所有 Prompt 都长得差不多。
            #     S_sq = S ** 2
            #     total_energy = torch.sum(S_sq)
            #     explained_var = S_sq / total_energy
                
            #     # 打印分析结果 (建议只在特定步数或 Task 结束时打印，避免刷屏)
            #     print(f"\n[Task {taskid} SVD Analysis]")
            #     print(f"  > Effective Rank: {effective_rank.item():.4f}") # 越高越好
            #     print(f"  > Top-3 Singular Values: {S[:3].cpu().detach().numpy()}")
            #     print(f"  > Top-3 Explained Var: {explained_var[:3].cpu().detach().numpy()}") # 越均匀越好
